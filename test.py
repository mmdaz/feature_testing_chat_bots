...
def step_impl(context):
    context.first_step = context.test_controoler.main_menu()
...
def step_impl(context):
    context.second_state = context.test_controoler.first_step()


@then('bot send user text')
def step_impl(context):
    assert context.second_step == context.test_controller.states_dict["step_2"]
